...
import { useROIService, RevenueItem as DBRevenueItem } from '../../lib/services/roiService';
...
export function RevenueTab({ roiCalculationId }: RevenueTabProps) {
  const [collapsedSections, setCollapsedSections] = useState<Record<string, boolean>>({});
  const [revenueData, setRevenueData] = useState<DBRevenueItem[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [totalEstimate, setTotalEstimate] = useState(0);
  const [totalActual, setTotalActual] = useState(0);

  const { getRevenue, addRevenueItem, updateRevenueItem, deleteRevenueItem } = useROIService();
  ...
  useEffect(() => {
    const loadRevenueData = async () => {
      ...
      setIsLoading(true);
      try {
        const items = await getRevenue(roiCalculationId);
    ...
    return (
      <div className="flex items-center justify-center min-h-96 p-8">
        <div className="text-center">
          <p className="text-gray-500 text-lg">Please select a project to view revenue data</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex items-center justify-center min-h-96 p-8">
        <div className="text-center">
          ...
          <p className="text-gray-500 text-lg">Loading revenue data...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-xs text-gray-500 uppercase">TOTAL REVENUE (ESTIMATE)</p>
                <p className="text-xl font-bold text-gray-800 mt-1">{formatCurrency(totalEstimate)}</p>
              </div>
              <DollarSign className="w-8 h-8 text-green-500 bg-green-100 p-2 rounded-full" />
            </div>
          </div>
          <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-xs text-gray-500 uppercase">TOTAL REVENUE (ACTUAL)</p>
                <p className="text-xl font-bold text-gray-800 mt-1">{formatCurrency(totalActual)}</p>
              </div>
              <DollarSign className="w-8 h-8 text-blue-500 bg-blue-100 p-2 rounded-full" />
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="bg-slate-50 p-6 rounded-lg mb-8 shadow-sm border border-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {Object.keys(collapsedSections).map((category, index) => {
                ...
                return (
                  <div key={category} className="bg-white p-4 rounded-lg shadow-sm border-l-4 border-blue-500">
                    <div className="flex justify-between items-start">
                      <div>
                        <p className="text-xs text-gray-500 uppercase">{category}</p>
                        <p className="text-lg font-bold text-gray-800 mt-1">{formatCurrency(categoryTotal)}</p>
                      </div>
                      <DollarSign className="w-6 h-6 text-blue-500 bg-blue-100 p-1 rounded-full" />
                    </div>
                  </div>
                );
              })}
        ...
        <div className="bg-slate-50 p-6 rounded-lg shadow-sm border border-slate-200">
          <div className="flex justify-between items-center mb-6">
            <div className="flex items-center space-x-4">
              <h2 className="text-xl font-semibold text-gray-800 flex items-center">
                <Receipt className="mr-2 text-green-500 w-6 h-6" />
          ...
          {revenueData.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-lg">
              ...
              <p className="text-gray-500 text-lg">No revenue items yet</p>
              <p className="text-gray-400 text-sm mt-2">Click "Add Revenue Item" to get started</p>
            </div>
          ) : (
                  ...
                  <div key={category} className="bg-white rounded-lg">
                    ...
                    >
                      <div className="flex items-center">
                        ...
                        <DollarSign className="mr-2 text-slate-600 w-5 h-5" />
                        <span>{category}</span>
                      </div>
                      <div className="flex items-center space-x-4">
                        ...
                        <div className="space-y-3">
                          {categoryData.map((item) => (
                            <div key={item.id} className="grid grid-cols-12 gap-x-4 items-center px-4 py-3 hover:bg-slate-100 rounded-md bg-white">
                              ...
                              <span className="text-sm text-blue-600 font-medium col-span-2 text-right">{formatCurrency(item.actual)}</span>
