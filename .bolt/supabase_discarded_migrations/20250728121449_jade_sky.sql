/*
  ...
  1. New Tables
    ...
    - `field_change_log` - Complete audit trail for all changes
  ...
  2. User Role Enhancement
    ...
    - Support roles: master, senior, mid, external, hr, admin, user

  3. Security
    - Enable RLS on all new tables
    - Master-only access to platform configuration
    - Read-only audit trail access for transparency

  4. Sample Data
    - Pre-populate with common platform fields
    - Ready-to-use field options
*/
...
-- Create RLS policies for platform_fields
...
-- Create RLS policies for field_options
...
-- Create RLS policies for field_change_log (read-only for audit purposes)
...
CREATE POLICY "Master users can create change log entries"
  ON field_change_log
  FOR INSERT
  TO authenticated
  WITH CHECK (
...
-- Create RLS policies for field_usage_tracking
...
INSERT INTO platform_fields (field_name, field_type, module, section, is_locked, sort_order) VALUES
  ('Project Status', 'dropdown', 'Projects', 'Project Info', true, 1),
  ('Client Classification', 'dropdown', 'Clients', 'Client Details', true, 1),
  ('Event Type', 'dropdown', 'Projects', 'Project Info', false, 2),
  ('Deal Stage', 'dropdown', 'Pipeline', 'Deal Details', false, 1),
  ('Client Tier', 'dropdown', 'Pipeline', 'Deal Details', false, 2)
