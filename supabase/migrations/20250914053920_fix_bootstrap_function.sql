-- Drop and recreate the bootstrap function with fixed syntax
DROP FUNCTION IF EXISTS create_bootstrap_master_user(TEXT, TEXT, TEXT);
...
BEGIN
  ...
  -- Insert or update user profile (bypassing RLS with SECURITY DEFINER)
  IF EXISTS (SELECT 1 FROM user_profiles WHERE id = target_user_id) THEN
