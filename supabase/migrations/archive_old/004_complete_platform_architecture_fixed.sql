-- ============================================
-- INTRAEXTRA PLATFORM - COMPLETE DATABASE ARCHITECTURE (FIXED)
-- Version: 1.0.1
-- Purpose: Comprehensive schema for all platform modules
-- Fix: Changed user_id references to id (user_profiles primary key)
-- ============================================
...
CREATE TABLE IF NOT EXISTS project_logistics_comments (
    ...
    mentions UUID[], -- Array of user_profile IDs mentioned
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS project_crew_rota (
    ...
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
    UNIQUE(project_id, user_profile_id, date)
);

-- Create indexes for crew
CREATE INDEX IF NOT EXISTS idx_project_assignments_project ON project_assignments(project_id);
CREATE INDEX IF NOT EXISTS idx_project_assignments_user ON project_assignments(user_profile_id);
...
CREATE INDEX IF NOT EXISTS idx_project_notifications_user ON project_notifications(user_profile_id);
...
CREATE POLICY "Users can view their notifications" ON project_notifications FOR SELECT USING (auth.uid() = (SELECT id FROM user_profiles WHERE id = user_profile_id));
...
DO $$
BEGIN
    RAISE NOTICE '✅ IntraExtra Platform Database Architecture v1.0.1 (FIXED) completed successfully!';
    ...
    RAISE NOTICE 'Fixed: All user references now use user_profiles(id) instead of user_id';
END $$;
